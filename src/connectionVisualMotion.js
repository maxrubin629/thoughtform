...
// This is the classic canvas connection response. Both routes use this exact
// helper so session reconciliation cannot drift into a similar-but-different
// wobble controller.
...
export function buildConnectionPopTransition(
  ...
  const nextNodes = nodes.map((node) => {
    if (node.id === source.id) return withWobble(
      ...
      source.y - hitY,
      sourceAmplitude,
      { startAt: impacts.source, travel: 0.13 },
    );
    if (node.id === target.id) return withWobble(
      node,
      target.x - hitX,
      target.y - hitY,
      targetAmplitude,
      { startAt: impacts.target, travel: 0.13 },
    );
    return node;
  });
  ...
  return {
    ...
    impacts,
    pop: {
      id: eventId ?? `pop:${edge.id ?? `${edge.from}:${edge.to}`}:${start}`,
      edgeId: edge.id,
      aId: source.id,
      bId: target.id,
